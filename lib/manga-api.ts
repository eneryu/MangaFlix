...
function parseManga(data: any, coverFileName?: string): Manga {
    ...
    return {
        id: data.id,
        title: titles.en || titles['ja-ro'] || Object.values(titles)[0] as string || 'Untitled',
        titleAr,
        titleJa: titles.ja,
        description: descriptions.en || Object.values(descriptions)[0] as string || '',
        descriptionAr: descriptions.ar,
        coverUrl: coverFileName ? getCoverUrl(data.id, coverFileName) : '/placeholder-cover.jpg',
        status: attrs.status || 'ongoing',
        year: attrs.year,
        genres,
        author: data.relationships?.find((r: any) => r.type === 'author')?.attributes?.name,
        artist: data.relationships?.find((r: any) => r.type === 'artist')?.attributes?.name
    };
}
...
export async function getMangaChapters(mangaId: string, languages = ['en', 'ar']): Promise<Chapter[]> {
    try {
        ...
        return response.data.data.map((ch: any) => ({
            ...
            scanlationGroup: ch.relationships?.find((r: any) => r.type === 'scanlation_group')?.attributes?.name
        }));
    } catch (error) {
...
// Build full page URL
export function getPageUrl(baseUrl: string, hash: string, filename: string, quality: 'data' | 'data-saver' = 'data'): string {
